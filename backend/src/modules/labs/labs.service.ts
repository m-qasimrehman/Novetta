...
export class LabsService {
  ...
  async listTests(category?: string, q?: string) {
  ...
  async listPackages(labCenterId?: string) {
  ...
  async bookTest(patientId: string, dto: {
    testId?: string
    packageId?: string
    labCenterId?: string
    ...
    if (dto.testId) {
      ...
      if (!pkg) throw new NotFoundException('Lab package not found')
  ...
  async myBookings(patientId: string) {
  ...
  async cancelBooking(id: string, patientId: string) {
