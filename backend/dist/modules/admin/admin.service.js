...
let AdminService = class AdminService {
    ...
    // ── Overview Stats ──────────────────────────────────────────────────────────
    async stats(userId, dateFrom, dateTo) {
    ...
    // ── Users ───────────────────────────────────────────────────────────────────
    async listUsers(userId, page = 1, limit = 20, role, q) {
        await this.assertAdmin(userId);
    ...
    async toggleUserActive(adminId, targetId) {
    ...
    async verifyDoctor(adminId, doctorId) {
        await this.assertAdmin(adminId);
    ...
    async updateCoupon(adminId, couponId, data) {
        await this.assertAdmin(adminId);
    ...
    async listAllAppointments(userId, status, page = 1, limit = 20) {
        await this.assertAdmin(userId);
        const skip = (page - 1) * limit;
    ...
    // ── Orders Oversight ────────────────────────────────────────────────────────
    async listAllOrders(userId, type = 'pharmacy', page = 1, limit = 20) {
        await this.assertAdmin(userId);
        const skip = (page - 1) * limit;
        if (type === 'lab') {
            const [data, total] = await Promise.all([
    ...
    async togglePharmacyActive(adminId, pharmacyId) {
        await this.assertAdmin(adminId);
    ...
    async toggleLabActive(adminId, centerId) {
        await this.assertAdmin(adminId);
        const center = await this.prisma.labCenter.findUnique({ where: { id: centerId } });
        if (!center)
            throw new common_1.NotFoundException('Lab center not found');
